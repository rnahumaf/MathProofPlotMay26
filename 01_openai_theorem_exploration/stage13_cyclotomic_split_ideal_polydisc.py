...
"""
Stage 13: Cyclotomic split-prime ideal fibers with Minkowski polydisc filtering.

What changed from Stage 12
--------------------------
Stage 12 constructed same-relative-norm fibers in K = Q(zeta_m), but it did
not apply the product-of-discs / hidden-embedding window. Without that window,
the point set is basically a hypercube of subset sums.

Stage 13 adds the geometric part of the proof:

    1. Build q's from split prime ideal choices in K = Q(zeta_m).
    2. Group by same relative norm q*c(q)=mu.
    3. For each embedding sigma, normalize q by |sigma(q)|.
       Thus every selected q is a unit vector in every complex embedding.
    4. Build subset sums.
    5. Keep only sums inside a product of disks:
           max_sigma |sum_j eps_j sigma(q_j)/|sigma(q_j)|| <= R.
    6. Project the surviving points to one complex embedding.
    7. Count unit-distance edges from one-bit changes.

This is now much closer to the proof architecture:
    split primes -> same norm fiber -> Minkowski product-of-discs -> 2D projection.

Default behavior:
    - summary CSV only
    - no huge detail files unless --save-details
    - no plot unless --plot-best
"""
...
def primes_split_completely_in_cyclotomic(m: int, count: int) -> list[int]:
    """In Q(zeta_m), unramified p splits completely iff p ≡ 1 mod m."""
    primes = []
    n = 2
...
def try_principal_generator(K, I):
    """Sage-version-tolerant attempt to extract a principal generator."""
    try:
...
def embedding_values(K, embeddings, z):
...
def build_split_ideal_generators(m: int, split_prime_count: int, max_choices: int, verbose: bool):
    ...
    for mask in range(total_choices):
        ...
        cq = conj(q)
        mu = q * cq
        values = embedding_values(K, embeddings, q)
        ...
        q_rows.append(
            {
                ...
                "max_abs_all_embeddings": float(max(abs_values)),
...
def normalized_embedding_matrix(selected: list[dict]) -> np.ndarray:
    """
    Matrix U[j,k] = sigma_k(q_j)/|sigma_k(q_j)|.

    Every row has unit modulus in every embedding coordinate.
    """
    rows = []
    for row in selected:
        vals = row["embedding_values"]
        normed = []
        for v in vals:
            av = abs(v)
            if av == 0:
                raise RuntimeError("Unexpected zero embedding value for q.")
            normed.append(v / av)
        rows.append(normed)
    return np.array(rows, dtype=complex)


def build_polydisc_subset_points(
    selected: list[dict],
    ...
    # DP for numeric and exact subset sums.
    ...
    for mask in range(raw_total):
        ...
        values = numeric_sums[mask]
        principal_abs = abs(values[0])
...
def evaluate_group(
    ...
    point_rows, mask_to_point_id, raw_total, raw_kept, collisions = build_polydisc_subset_points(
        selected=selected,
...
def plot_result(output_png: Path, point_rows, edge_rows, summary, max_plot_edges: int):
    ...
    cbar.set_label("max embedding |sum|")

    ax.autoscale()
    ax.set_aspect("equal", adjustable="box")
    ax.grid(True, linewidth=0.3, alpha=0.25)
    ax.set_xlabel("Re(normalized sigma_1 sum)")
    ax.set_ylabel("Im(normalized sigma_1 sum)")
    ax.set_title("Cyclotomic split-prime ideal fiber + polydisc")
    ...
    fig.suptitle(
        "Stage 13: split-prime ideal same-norm fiber with Minkowski polydisc\n"
...
def main() -> None:
    ...
    parser.add_argument("--polydisc-radii", type=str, nargs="+", default=["2", "3", "4", "none"])
    ...
    parser.add_argument("--outdir", type=Path, default=Path("stage13_cyclotomic_split_ideal_polydisc_output"))
                ...
                for tc in args.translation_counts:
                    ...
                    for polyr in polydisc_radii:
                        for prinr in principal_radii:
                            ...
                            print(
                                f"  fiber={len(group)} tc={tc} polyR={polyr} prinR={prinr}: "
                            ...
                            if best_summary is None or (
                                ...
                                best_context = (m, spc, mu_key, group, tc, polyr, prinr)

    # Write summaries.
    if global_summaries:
        all_fields = sorted({k for row in global_summaries for k in row.keys()})
        write_csv(args.outdir / "stage13_polydisc_summary.csv", global_summaries, all_fields)
    else:
        write_csv(args.outdir / "stage13_polydisc_summary.csv", [], ["status"])

    if factor_summaries:
        all_fields = sorted({k for row in factor_summaries for k in row.keys()})
        write_csv(args.outdir / "stage13_factor_summary.csv", factor_summaries, all_fields)
    else:
        write_csv(args.outdir / "stage13_factor_summary.csv", [], ["m"])

    if fiber_summaries:
        all_fields = sorted({k for row in fiber_summaries for k in row.keys()})
        write_csv(args.outdir / "stage13_fiber_summary.csv", fiber_summaries, all_fields)
    else:
        write_csv(args.outdir / "stage13_fiber_summary.csv", [], ["m"])

    if best_summary is None:
        print("No successful evaluated fiber. Inspect summary files.")
        print(f"Wrote: {args.outdir / 'stage13_polydisc_summary.csv'}")
        print(f"Wrote: {args.outdir / 'stage13_factor_summary.csv'}")
        print(f"Wrote: {args.outdir / 'stage13_fiber_summary.csv'}")
        return
    ...
    if args.save_details or args.plot_best:
        m, spc, mu_key, group, tc, polyr, prinr = best_context
        ...
        stem = (
            f"best_m{m}_spc{spc}_tc{tc}"
            f"_polyR{'none' if polyr is None else polyr}"
            f"_prinR{'none' if prinr is None else prinr}"
        ).replace(".", "p")
